...
**Что страхует переключение.** `src/api/__tests__/contract.test.ts` — один набор из 15 проверок, который прогоняется по обоим адаптерам сразу: мок и HTTP (второй обслуживается MSW). Если реализации разойдутся в поведении — например, сервер вернёт `null` вместо `404` — тест упадёт до того, как это увидит пользователь.
...
126 юнит- и компонентных тестов. Покрытие слоёв `api`, `domain`, `hooks`, `lib` — 96% строк при пороге 80%.
