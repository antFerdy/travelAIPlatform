...
Правила, по которым это устроено, вынесены в [`ai-rules/frontend_adilbek.md`](ai-rules/frontend_adilbek.md) и частично проверяются автоматически: ESLint запрещает прямой `fetch` вне адаптера и импорт адаптера из компонентов.
